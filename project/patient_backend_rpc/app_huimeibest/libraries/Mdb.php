 ...
 * @package		CodeIgniter
 * @author		Soone
 * @copyright	Copyright (c) 2015, Soone.
 * @license		http://codeigniter.com/user_guide/license.html
 * @link		http://huimeibest.com
...
class Mdb{
	...
	*/
	...
	public function likes($field = "", $value = "", $flags = "i", $enable_start_wildcard = TRUE, $enable_end_wildcard = TRUE) {
	 	...
	 	$regex = "/$value/$flags";
		$regexOb = new MongoRegex($regex);
	 	$this->wheres['$or'][] = array($field=>$regexOb);
	 	return ($this);
	 }
	...
	*/
	 ...
	 public function get($collection = "",$idStr = false) {
	 	if (empty($collection)) {
		...
		foreach ($documents as $doc) {
			if($idStr){ $doc['_id'] = (string)$doc['_id']; }
	 		$returns[] = $doc;
	 	}
	...
	public function group($tablename, $keys, $initial, $reduce, $options =array()){
	 	if (empty($tablename)) {return array(); }
		try{
			$result =  $this->db->{$tablename}->group($keys,$initial,$reduce,$options);
		}catch(Exception $e){
			//记录错误
			//echo $e;
			return array();
	...
	public function aggregate($tablename,$pipeline,$options=array()){
	 	if (empty($tablename)) {return array(); }
		try{
			$result =  $this->db->{$tablename}->aggregate($pipeline,$options);
		}catch(Exception $e){
			//记录错误
			//echo $e;
			return array();
	 ...
	 public function getRef($collection = "",$ref) {
	 	$documents = $this->db->{$collection}->getDBRef($ref);
	 	$this->_clear();
		$returns = $documents;
	 	if ($this->CI->config->item('mongo_return') == 'object') {
			return (object)$returns;
	...
	*/
	...
	public function insert($collection = "", $insert = array()) {
		...
		try {
			$this->db->{$collection}->insert($insert, array($this->query_safety	 => 1));
			if (isset($insert['_id'])) {
				return ($insert['_id']);
			} else {
				return (FALSE);
	...
	public function batchinsert($collection = "", $insert = array()) {
		...
		try {
			$this->db->{$collection}->batchInsert($insert, array($this->query_safety => 1,'continueOnError' => true));
			return true;
		}catch (MongoCursorException $e) {
			return false;
		}
	...
	*/
	...
	public function update($collection = "", $data = array(), $options = array()) {
		...
		try {
			$options = array_merge($options, array($this->query_safety =>1, 'multiple' => TRUE));
	...
	public function updateset($collection = "", $data = array(), $options = array()) {
		try {
			$options = array_merge($options, array($this->query_safety =>1, 'multiple' => FALSE));
			$w = $this->db->{$collection}->update($this->wheres, $data, $options);
			$this->_clear();
			return $w;
		} catch (MongoCursorException $e) {
			return false;
	...
	*/
	...
	public function delete($collection = "") {
		try {
			$this->db->{$collection}->remove($this->wheres, array($this->query_safety =>1, 'justOne' => TRUE));
			$this->_clear();
			return (TRUE);
		} catch (MongoCursorException $e) {
			return false;
	...
	*/
	...
	private function connection_string() {
		...
		$this->host	= trim($this->CI->config->item('mdb_host'));
		$this->port = trim($this->CI->config->item('mdb_port'));
		$this->user = trim($this->CI->config->item('mdb_user'));
		$this->pass = trim($this->CI->config->item('mdb_pass'));
		$this->dbname = trim($this->CI->config->item('mdb_db'));
		$this->persist = trim($this->CI->config->item('mdb_persist'));
		$this->persist_key = trim($this->CI->config->item('mdb_persist_key'));
		$this->query_safety = trim($this->CI->config->item('mdb_query_safety'));
