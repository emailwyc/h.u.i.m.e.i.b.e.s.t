...
if (! function_exists ( 'checkAuth' )) {
	function checkAuth()
	{
		if(isset($_SESSION['aid']) && $_SESSION['aid'] && isset($_SESSION['st']) && $_SESSION['st'] && $_SESSION['from']=="fesco") {
			return $_SESSION['aid'];
...
//检测用户时候注册
function checkAuth1() {
	if(isset($_SESSION['pid']) && $_SESSION['pid'] && $_SESSION['from']=="fesco") {
		return $_SESSION['pid'];
	} else return false;
}
function getPatientOpenid() {
	if(isset($_SESSION['aid']) && $_SESSION['aid'] && $_SESSION['from']=="fesco") {
		return $_SESSION['aid'];
	} else return false;
}
...
function checkAuth2() {
		if(!empty($_SESSION['aid']) && !empty($_SESSION['st'])  && !empty($_SESSION['pid']) && $_SESSION['from']=="fesco") {
			return array("aid"=>$_SESSION['aid'],"pid"=>$_SESSION['pid']);
		}else return false;
}
function checkAuth3()
{
	if(isset($_SESSION['pid']) && $_SESSION['pid'] && isset($_SESSION['st']) && $_SESSION['st'] && $_SESSION['from']=="fesco") {
		return $_SESSION['pid'];
	} else{
		$selfUrl = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
		redirect('/register/index?redirect='.urlencode($selfUrl));
