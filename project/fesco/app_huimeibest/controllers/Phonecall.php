...
class Phonecall extends CI_Controller {
	...
	public function __construct(){
		parent::__construct();
		$userid =checkPatientAuth();
		$this->openid = $userid['aid'];
	...
	public function order($dayid="",$str="") {
		...
		//得到用户剩余使用次数
		$fescoInfo = $this->Common_model->getInfo('fesco_user',array('_id'=>getMdbId($this->openid),'status'=>1));
		if(empty($fescoInfo)){ show_error("该账号已经停用，请联系管理员!"); }

		if(isset($btn['btn1'])){
			$patInfo = $this->user_model->getInfo('patient_family',array('_id'=>getMdbId($btn['patient'])));
			if(empty($patInfo)) { show_error("未找到就诊人信息，请检查！");	}
			...
			if((int)$fescoInfo['ticket_overplus']<=0){ show_error("该账号已经没有使用次数!"); }
			$insertData = array(
					"doctor"=>$docref,
					"patient"=>$patref,
			...
			if($check){
				if($price_t<=0){
					...
					exit;
				}else{ show_error("订单出错，请联系管理员!"); }
			}else{
				show_error("下单失败，请联系管理员!");
			}
		}
