...
<div class="ys-info">
  <div class="top">
    ...
    <div class="zxgh cw8"><img src="/ui/images/ico3-gh.png" alt="" /><span>加号量 <em><?=@$doctor['reg_num']?></em></span></div>
  </div>
</div>
...
<div class="jzr-form pb90">
  ...
  <div class="info fs14 ch7 lh15">
    <div>就诊科室：<?=@$doctor['department'];?></div>
    <div>就诊医院：<?=@$day['location']['hospital']?><?php if($day['location']['branch']){ echo "(".$day['location']['branch'].")";}?></div>
    <div>就诊地点：<?=@$day['location']['info']?><?php if(!empty($day['location']['address'])&&!empty($day['location']['info'])){ echo "(".$day['location']['address'].")";}else{ echo $day['location']['address'];}?></div>
    <div>就诊时间：<?=@$day['date']?>  <?=@$week?>  <?php $temTim = explode(',',$day['interval']); if($temTim['1']<="12:00"){?>上午<?php }else{?>下午<?php }?></div>
  </div>
  ...
  <div class="textarea">
    <textarea name="message" class="lh15 fs13" maxlength="1000"  placeholder="请简述您的【个人信息、主要病史、过敏史、身体基本状况】。请就此次预约加号提供您的【疾病状况】。包括：疾病起因、症状、持续时间、进行过何种检查和治疗。请在就诊时携带您的相关病历及检查、诊断报告。（请输入10-1000字）"></textarea>
  </div>
...
<div class="jzr-submit bb">
  <div class="price"><span class="fs13"><?php if(@$day['price']>0){?><?=@$day['price']?>元／次<?php }else{?>义诊<?php } ?></span><em></em></div>
  <div class="txt ch4">支付完成后，请直接找医生就诊，由医生安排加号</div>
...
<div class="jzr-select bb zi10 fs14 tc">
  <div>
    ...
    <a class="soone" href="javascript:void(0)" dataid="<?=@$v['_id']?>" datasex="<?=@$v['gender']?>" dataage="<?=@$v['age']?>"><?=@$v['name']?></a>
    <?php }?>
  </div>
...
$(document).ready(function(){
	...
	ajaxjzr();//请求优惠券列表
	ajaxcoupons("3","<?=@$day['price']?>");//请求优惠券列表
	updateimg();//图片处理
});
