...
if __name__ == "__main__":
    ...
    if len(ACCESS_ID) == 0 or len(SECRET_ACCESS_KEY) == 0:
        print("Please make sure ACCESS_ID and SECRET_ACCESS_KEY are correct in ",
              __file__, ", init are empty!")
        exit(0)
    ...
    timeout = 60
    url_with_auth = oss.sign_url_auth_with_expire_time(
        method, url, headers, resource, timeout)
        ...
        for i in object_group_index:
            if len(i) == 4 and len(i[1]) > 0:
                ...
                if res.status != 204:
                    ...
                    print(
                        "delete part ", part_name, " in bucket:", bucket, " ok")
    print(sep)
    ...
    part_number = "1"
    res = oss.upload_part_from_string(
        bucket, object, data, upload_id, part_number)
    if (res.status // 100) == 2:
