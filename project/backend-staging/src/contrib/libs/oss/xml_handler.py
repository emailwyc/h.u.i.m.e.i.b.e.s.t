...
class Bucket:
    ...
    def show(self):
        print("Name: %s\nCreationDate: %s\nLocation: %s" %
              (self.name, self.creation_date, self.location))
...
class GetObjectGroupIndexXml:
    ...
    def list(self):
        ...
        for i in self.index_list:
            index_list.append(
                (i.part_num, i.object_name, i.object_size, i.etag))
        return index_list

    def show(self):
        print("Bucket: %s\nObject: %s\nEtag: %s\nObjectSize: %s" %
              (self.bucket, self.key, self.etag, self.file_length))
        print("\nPart list:")
...
class GetBucketXml:
    ...
    def list(self):
        ...
        pl = []
        for c in self.content_list:
            cl.append((c.key, c.last_modified, c.etag, c.size,
                       c.owner.id, c.owner.display_name, c.storage_class))
        for p in self.prefix_list:
            pl.append(p)
...
class GetBucketAclXml:
    ...
    def show(self):
        print("Owner Name: %s\nOwner ID: %s\nGrant: %s" %
              (self.owner.id, self.owner.display_name, self.grant))
...
class GetMultipartUploadsXml:
    ...
    def __init__(self, xml_string):
        ...
        self.next_upload_id_marker = get_tag_text(
            self.xml, 'NextUploadIdMarker')
        self.delimiter = get_tag_text(self.xml, 'Delimiter')
        self.prefix = get_tag_text(self.xml, 'Prefix')
...
class GetPartsXml:
    ...
    def __init__(self, xml_string):
        ...
        self.next_part_number_marker = get_tag_text(
            self.xml, 'NextPartNumberMarker')
