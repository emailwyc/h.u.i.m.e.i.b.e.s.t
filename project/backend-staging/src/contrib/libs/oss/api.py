...
class OssAPI:
    ...
    def get_connection(self, tmp_host=None, timeout=10):
        host = ''
        port = 80
        if not tmp_host:
            tmp_host = self.host
    ...
    def sign_url_auth_with_expire_time(self, method, url, headers=None, resource="/", timeout=60, params=None):
        ...
        auth_value = get_assign(
            self.secret_access_key, method, headers, resource, None, self.debug).decode('utf-8')
    ...
    def http_request(self, method, bucket, object, headers=None, body='', params=None, timeout=10):
        '''
        ...
        while retry > 0:
            ...
            res = self.http_request_with_redirect(
                method, tmp_bucket, tmp_object, tmp_headers, body, tmp_params, timeout=timeout)
    ...
    def http_request_with_redirect(self, method, bucket, object, headers=None, body='', params=None, timeout=10):
        '''
        Send http request of operation
        ...
        headers['Date'] = date
        headers['Authorization'] = self._create_sign_for_normal_auth(
            method, headers, resource)
        headers['User-Agent'] = self.agent
        if check_bucket_valid(bucket) and not is_ip(self.host):
            conn = self.get_connection(headers['Host'], timeout=timeout)
        else:
    ...
    def get_bucket(self, bucket, prefix='', marker='', delimiter='', maxkeys='', headers=None, timeout=10):
        '''
        List object that in bucket
        '''
        return self.list_bucket(bucket, prefix, marker, delimiter, maxkeys, headers, timeout=timeout)

    def list_bucket(self, bucket, prefix='', marker='', delimiter='', maxkeys='', headers=None, timeout=10):
        '''
        List object that in bucket
        ...
        return self.http_request(method, bucket, object, headers, body, params, timeout=timeout)
    ...
    def put_object_from_string(self, bucket, object, input_content, content_type=DefaultContentType, headers=None, params=None):
        ...
        res = self.put_object_from_fp(
            bucket, object, fp, content_type, headers, params)
        fp.close()
        return res
    ...
    def put_object_from_fp(self, bucket, object, fp, content_type=DefaultContentType, headers=None, params=None):
        ...
        conn = self._open_conn_to_put_object(
            bucket, object, filesize, content_type, headers, params)
        totallen = 0
    ...
    def put_object_from_file_given_pos(self, bucket, object, filename, offset, partsize, content_type='', headers=None, params=None):
        ...
        if not content_type:
            content_type = get_content_type_by_filename(filename)
        conn = self._open_conn_to_put_object(
            bucket, object, partsize, content_type, headers, params)
        left_len = partsize
        while True:
            if left_len <= 0:
                break
    ...
    def upload_large_file(self, bucket, object, filename, thread_num=10, max_part_num=1000, headers=None):
        ...
        while(retry_times >= 0):
            try:
                ...
                for i in range(0, thread_num):
                    ...
                    begin = i * step
                    oss = OssAPI(
                        self.host, self.access_id, self.secret_access_key)
                    current = PutObjectGroupWorker(
                        oss, bucket, filename, part_msg_list[begin:end], self.retry_times)
        ...
        if -1 >= retry_times:
            print("after retry %s, failed, upload large file failed!" %
                  retry_times)
            return
    ...
    def copy_object(self, source_bucket, source_object, target_bucket, target_object, headers=None):
        ...
        headers[
            'x-oss-copy-source'] = "/%s/%s" % (source_bucket, source_object)
        method = 'PUT'
        body = ''
        params = {}
    ...
    def multi_upload_file(self, bucket, object, filename, upload_id='', thread_num=10, max_part_num=10000, headers=None, params=None):
        ...
        logger.info("bucket:%s, object:%s, upload_id is: %s, split_number:%d" % (
            bucket, object, upload_id, len(part_msg_list)))
        ...
        while(upload_retry_times >= 0):
            ...
            while(retry_times >= 0):
                ...
                try:
                    for i in range(0, thread_num):
                        ...
                        oss = OssAPI(
                            self.host, self.access_id, self.secret_access_key)
                        current = UploadPartWorker(oss, bucket, object, upload_id, filename, part_msg_list[
                                                   begin:end], uploaded_part_map, self.retry_times)
                        threadpool.append(current)
                        current.start()
                    for item in threadpool:
                        item.join()
            ...
            if -1 >= retry_times:
                raise Exception(
                    "-2, after retry %s, failed, multi upload part failed! upload_id:%s" % (self.retry_times, upload_id))
            # get xml string that contains msg of part
            part_msg_xml = create_part_xml(part_msg_list)
            # complete upload
            res = self.complete_upload(
                bucket, object, upload_id, part_msg_xml, headers, params)
            if res.status == 200:
                break
            upload_retry_times -= 1
        if upload_retry_times < 0:
            raise Exception(
                "-3, after retry %s, failed, multi upload file failed! upload_id:%s" % (self.retry_times, upload_id))
        return res
    ...
    def list_objects(self, bucket, prefix=''):
        ...
        object_list = []
        oss = OssAPI(self.host, self.access_id, self.secret_access_key)
        (object_list, marker_output) = get_instance.get_object_in_bucket(
            oss, bucket, marker_input, prefix)
        return object_list
