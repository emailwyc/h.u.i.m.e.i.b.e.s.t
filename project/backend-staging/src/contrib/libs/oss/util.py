...
def get_assign(secret_access_key, method, headers=None, resource="/", result=None, debug=DEBUG):
    ...
    h = hmac.new(
        bytes(secret_access_key, 'utf-8'), bytes(string_to_sign, 'utf-8'), sha)
...
def append_param(url, params):
    ...
    for k, v in list(params.items()):
        ...
        if v is not None and v != '':
            l.append('%s=%s' %
                     (urllib.parse.quote(k), urllib.parse.quote(str(v))))
        elif k == 'acl':
...
def clear_all_objects_in_bucket(oss_instance, bucket, delete_marker="", delete_upload_id_marker="", debug=False):
    ...
    maxkeys = 1000
    delete_all_objects(
        oss_instance, bucket, prefix, delimiter, delete_marker, maxkeys, debug)
    delete_all_parts(
        oss_instance, bucket, delete_marker, delete_upload_id_marker, debug)
    res = oss_instance.delete_bucket(bucket)
    if (res.status // 100 != 2 and res.status != 404):
        print("clear_all_objects_in_bucket: delete bucket:%s fail, ret:%s, request id:%s" % (
            bucket, res.status, res.getheader("x-oss-request-id")))
        return False
    return True
...
def delete_all_objects(oss_instance, bucket, prefix="", delimiter="", delete_marker="", maxkeys=1000, debug=False):
    ...
    while 1:
        ...
        if object_list:
            ...
            if res.status / 100 != 2:
                if marker:
                    print("delete_all_objects: batch delete objects in bucket:%s fail, ret:%s, request id:%s, first object:%s, marker:%s" % (
                        bucket, res.status, res.getheader("x-oss-request-id"), object_list[0], marker))
                else:
                    print("delete_all_objects: batch delete objects in bucket:%s fail, ret:%s, request id:%s, first object:%s" % (
                        bucket, res.status, res.getheader("x-oss-request-id"), object_list[0]))
            else:
                if debug:
                    delete_obj_num += len(object_list)
                    if marker:
                        print("delete_all_objects: Now %s objects deleted, marker:%s" % (
                            delete_obj_num, marker))
                    else:
                        print("delete_all_objects: Now %s objects deleted" %
                              (delete_obj_num))
        if len(marker) == 0:
            break
...
def delete_all_parts(oss_instance, bucket, delete_object_marker="", delete_upload_id_marker="", debug=False):
    ...
    while 1:
        res = oss_instance.get_all_multipart_uploads(
            bucket, key_marker=marker, upload_id_marker=id_marker)
        if res.status != 200:
            break
        body = res.read()
        ...
        for i in fl:
            ...
            if (res.status // 100 != 2 and res.status != 404):
                print("delete_all_parts: cancel upload object:%s, upload_id:%s FAIL, ret:%s, request-id:%s" %
                      (object, i[1], res.status, res.getheader("x-oss-request-id")))
            else:
                delete_mulitipart_num += 1
                if debug:
                    print("delete_all_parts: cancel upload object:%s, upload_id:%s OK\nNow %s parts deleted." % (
                        object, i[1], delete_mulitipart_num))
        if hh.is_truncated:
...
def pgfs_clear_all_objects_in_bucket(oss_instance, bucket):
    ...
    for i in b.object_list:
        ...
        if (res.status // 100 != 2):
            print("clear_all_objects_in_bucket: delete object fail, ret is:",
                  res.status, "bucket is:", bucket, "object is: ", i)
            return False
        else:
            pass
    ...
    res = oss_instance.delete_bucket(bucket)
    if (res.status // 100 != 2 and res.status != 404):
        print("clear_all_objects_in_bucket: delete bucket fail, ret is: %s, request id is:%s" % (
            res.status, res.getheader("x-oss-request-id")))
        return False
    return True
    ...
    if res.status == 200:
        ...
        for i in object_group_index:
            if len(i) == 4 and len(i[1]) > 0:
                ...
                if res.status != 204:
                    print(
                        "delete part ", part_name, " in bucket:", bucket, " failed!")
                    return False
    else:
        return False
    return True
...
class GetAllObjects:
    ...
    def get_all_object_in_bucket(self, oss, bucket="", marker="", prefix=""):
        marker2 = ""
        while True:
            (object_list, marker) = self.get_object_in_bucket(
                oss, bucket, marker2, prefix)
            marker2 = marker
...
def get_all_upload_id_list(oss, bucket):
    ...
    id_marker = ""
    while True:
        res = oss.get_all_multipart_uploads(
            bucket, key_marker=marker, upload_id_marker=id_marker)
        if res.status != 200:
...
def get_upload_id_list(oss, bucket, object):
    ...
    while True:
        res = oss.get_all_multipart_uploads(
            bucket, prefix=object, key_marker=marker, upload_id_marker=id_marker)
        if res.status != 200:
            break
        body = res.read()
        hh = GetMultipartUploadsXml(body)
...
def get_part_list(oss, bucket, object, upload_id, max_part=""):
    ...
    marker = ""
    while True:
        res = oss.get_all_parts(
            bucket, object, upload_id, part_number_marker=marker, max_parts=max_part)
        if res.status != 200:
            break
        body = res.read()
        h = GetPartsXml(body)
        ...
        while True:
            ...
            while True:
                try:
                    if retry_times <= 0:
                        break
                    res = self.oss.delete_objects(
                        bucket, object_list[begin:end])
                    if res.status // 100 == 2:
                        is_fail = False
                        break
                except:
                    ...
                    time.sleep(1)
            if is_fail:
                print("delete object_list[%s:%s] failed!, first is %s" % (
                    begin, end, object_list[begin]))
            begin = end
            ...
            if len(part) == 5:
                ...
                while True:
                    try:
                        if retry_times <= 0:
                            break
                        res = self.oss.put_object_from_file_given_pos(
                            bucket, object_name, self.file_path, offset, partsize)
                        if res.status != 200:
                            print(
            ...
            if len(part) == 5:
                ...
                while True:
                    try:
                        if retry_times <= 0:
                            break
                        res = self.oss.upload_part_from_file_given_pos(
                            bucket, object, self.file_path, offset, partsize, self.upload_id, part_number)
                        if res.status != 200:
                            self.logger.warn(
                                "Upload %s/%s from %s, failed! ret is:%s." % (bucket, object, self.file_path, res.status))
                            self.logger.warn("headers:%s" % res.getheaders())
                            retry_times = retry_times - 1
                            time.sleep(1)
                        else:
                            self.logger.info(
                                "Upload %s/%s from %s, OK! ret is:%s." % (bucket, object, self.file_path, res.status))
                            break
                    except:
                        retry_times = retry_times - 1
                        time.sleep(1)
...
class MultiGetWorker(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                res = self.oss.object_operation(
                    "GET", self.bucket, self.object, headers)
                if res.status == 206:
            ...
            if retry_times > self.retry_times:
                print("ERROR, reach max retry times:%s when multi get /%s/%s" %
                      (self.retry_times, self.bucket, self.object))
                break
...
def split_large_file(file_path, object_prefix="", max_part_num=1000, part_size=10 * 1024 * 1024, buffer_size=10 * 1024):
    ...
    if os.path.isfile(file_path):
        ...
        if file_size > part_size * max_part_num:
            part_size = (file_size + max_part_num - file_size %
