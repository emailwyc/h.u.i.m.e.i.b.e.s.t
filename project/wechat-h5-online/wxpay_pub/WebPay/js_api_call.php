...
$userOpenid = (string)@trim($_SESSION['aid']);
$userLoginSt = @$_SESSION['st'];
if(empty($userOpenid) || empty($userLoginSt)){ show_error("您还没有登录!"); }
...
$orderInfo = $Mdb->where(array('_id'=>$obj_id))->limit(1)->get("order");
if(!empty($orderInfo)){
	$orderInfo = $orderInfo[0];
}else{
	show_error("失效的订单,请检查!");
}
if($orderInfo['status']!="新订单" || $orderInfo['service']!=$params['type']){
	show_error("订单不符合规则!");
}
//更新订单
...
<head>
	...
	//						}
		...
		function ajax(id,num){ $.ajax({ type: "post", url: "/Json/payFail", dataType: "json",	data:{"order_id":id,"st":num}, success: function (data){ window.history.go(-1); }, error:function(msg){ window.history.go(-1); } }); }
	</script>
</head>
<body>
	</br></br></br></br>
