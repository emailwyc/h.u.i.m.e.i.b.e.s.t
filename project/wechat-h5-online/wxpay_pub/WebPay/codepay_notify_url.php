	...
	if($notify->checkSign() == TRUE)
	{
		...
		elseif($notify->data["result_code"] == "FAIL"){
			...
			if(!empty($rData['out_trade_no'])){
				...
				$info = $Mdb->where($where)->limit(1)->get('order_qrcode');
				if(!empty($info)){
					$info = $info[0];
					if($info['service']=="codepay" && $info['status']==0){
						//处理订单
						$updateOrder = array('$set'=>array('status'=>1,'updated_at'=>$cur_time,'pay_at'=>$cur_time,'openid'=>$rData['openid']));
						$check = $Mdb->where($where)->updateset('order_qrcode',$updateOrder);
						if($check){
						   	echo $returnXml;
							$url = $config['global_base_url']."/Callback/codepay_pay";
