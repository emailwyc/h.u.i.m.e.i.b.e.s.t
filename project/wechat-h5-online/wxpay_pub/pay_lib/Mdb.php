...
class Mdb{
	...
	*/
	...
	private function connect() {
		...
		try {
			...
			if($config['mongo_supress_connect_error']) {
				return false;
			} else {
				return false;
			}
	...
	*/
	...
	private function connection_string() {
		...
		$this->host	= trim($config['mdb_host']);
		$this->port = trim($config['mdb_port']);
		$this->user = trim($config['mdb_user']);
		$this->pass = trim($config['mdb_pass']);
		$this->dbname = trim($config['mdb_db']);
		$this->persist = trim($config['mdb_persist']);
		$this->persist_key = trim($config['mdb_persist_key']);
		$this->query_safety = trim($config['mdb_query_safety']);
