...
class User extends CI_Controller {
	...
	public function __construct(){
		parent::__construct();
		if(isset($_GET['auth']) && $_GET['auth']=="wx"){
			$userid = false;
		}else{
			$userid =checkAuth3();
		}
	...
	public function addPatient() {
		...
		if(isset($btn['btn1'])){
			...
			if(empty($data['msg'])){
				$where = array('openid'=>$this->uid,'name'=>$btn['name']);
			...
			if(empty($data['msg'])){
				...
				if(!empty($btn['isdefault'])){
					$where = array('openid'=>$this->uid,'name'=>array('$ne'=>$btn['name']));
	...
	public function checkPatient() {
		...
		$where = array('openid'=>$this->uid,'name'=>$name);
		$fname = $this->user_model->getInfo('patient_family',$where);
	...
	public function editPat($pid="") {
		...
		if(isset($btn['btn1']) && !empty($pid)){
			...
			if(empty($data['msg'])){
				...
				if(!empty($btn['isdefault'])){
					$where = array('openid'=>$this->uid,'_id'=>array('$ne'=>$pid1));
					$update = array('isdefault'=>0);
					$this->user_model->updateRecord('patient_family',$where,$update);
				}
